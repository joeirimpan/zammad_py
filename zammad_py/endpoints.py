...
class Endpoint(ABC):
    ...
    def _to_dict(self):
        ...
        allowed_attributes = [attr for attr in attributes if attr not in self.HIDDEN_ATTRIBUTES]
    ...
    def _get_changed_attributes(self):
        ...
        for attr in self.changed_attributes:
            if attr not in self.HIDDEN_ATTRIBUTES:
                changed_dict[attr] = getattr(self, attr)
        self.changed_attributes.clear()
        return changed_dict

class ZammadUser(Endpoint):
    def __init__(
        self,
        connection,
    ...
    def create(self):
        ...
        created_dict = self._connection.user.create(params=self._to_dict())
        for key, value in created_dict.items():
            setattr(self, key, value)

    def update(self):
        updated_dict = self._connection.user.update(self.id, params=self._get_changed_attributes())
        for key, value in updated_dict.items():
            setattr(self, key, value)

    def delete(self):
        self._connection.user.destroy(self.id)
